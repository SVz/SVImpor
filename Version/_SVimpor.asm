...
;                mov base_thunk,eax
                ...
                mov code_start,401000h
                mov code_end,409200h
                mov edx,pMemory_newthunk
                mov index_thunk,edx
                mov thunk_start,edx
                mov thunk_size,0

;                invoke GlobalAlloc,GMEM_MOVEABLE or GMEM_ZEROINIT,8200h
...
;                rep movsb
        ...
        not_valid:
;------------------------------------------- Patch Safe Code to return right address
                mov edi,v_add        
                mov ebx,[edi+20]
                mov edi,[ebx]
                add edi,0d7h
                ...
                mov buffer2,eax
                mov dword ptr [edi],0c308c483h  ;add esp,8
                                                ;ret
                call edx
            
                mov eax,[esp-0ch]               ;right address here
                mov right_rva,eax
...
fin:
    ...
    invoke GlobalFree,hMemory_code
                ...
                mov eax,pMemory_newthunk
                invoke wsprintf,addr textbuffer,offset template,eax
                invoke SetDlgItemText,hWnd,IDC_START,addr textbuffer

                mov eax,index_thunk
                invoke wsprintf,addr textbuffer,offset template,eax
                invoke SetDlgItemText,hWnd,IDC_END,addr textbuffer

                invoke GetDlgItemText,hWnd,IDC_RELOC,addr textbuffer,10
                invoke htodw,addr textbuffer
                add eax,index_thunk
                sub eax,pMemory_newthunk
                invoke wsprintf,addr textbuffer,offset template,eax
                invoke SetDlgItemText,hWnd,IDC_RELOC,addr textbuffer
                
safe_safe:
                push seh.PrevLink
                pop fs:[0]

                invoke SendMessage,hWnd,WM_COMMAND,IDC_GO,BN_CLICKED
                popad   
;------------------------------------------------------------------------------
